 ...
 *
 * 目前觀看時數採用留言驅動的重算模式：
 * - 每批留言成功入庫後，會依 viewer/channel/date 重新計算當天觀看秒數
 * - `allowOverwrite: true` 用於允許這條留言驅動流程覆寫當日資料
 * - `allowOverwrite: false` 時直接略過，避免非預期的背景流程寫入
 *
 * @param viewerId - 觀眾 ID
 * @param channelId - 頻道 ID
 * @param date - 日期（會被正規化到當天 00:00:00）
 * @param options.allowOverwrite - 是否允許覆蓋現有資料（預設 false）
 *   - `false` (預設): 跳過重算
 *   - `true`: 從訊息重新計算並覆蓋當日資料
 */
