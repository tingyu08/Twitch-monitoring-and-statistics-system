...
export class StreamerSettingsService {
   ...
   * 建立設定模板
   */
  async createTemplate(streamerId: string, data: CreateTemplateDto) {
    return templateService.create(streamerId, data);
  }
  ...
  async getTemplates(streamerId: string) {
    return templateService.findAll(streamerId);
  }

  async updateTemplate(streamerId: string, templateId: string, data: UpdateTemplateDto) {
    const updated = await templateService.update(templateId, streamerId, data);
    if (!updated) {
  ...
  async deleteTemplate(streamerId: string, templateId: string) {
    const deleted = await templateService.delete(templateId, streamerId);
    if (!deleted) {
      throw new Error("Template not found or permission denied");
    }
