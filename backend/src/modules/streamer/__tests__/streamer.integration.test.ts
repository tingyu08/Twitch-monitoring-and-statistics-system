...
jest.mock("../../auth/auth.middleware", () => ({
  requireAuth: (req: express.Request, _res: express.Response, next: express.NextFunction) => {
    (req as express.Request & { user?: { streamerId: string } }).user = {
...
describe("Streamer Integration Tests", () => {
  ...
  describe("GET /api/streamer/me/summary", () => {
    ...
    it("應該成功取得統計摘要（預設 30 天）", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/summary").expect(200);
    ...
    it("應該支援 7 天範圍查詢", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/summary?range=7d").expect(200);
    ...
    it("應該支援 90 天範圍查詢", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/summary?range=90d").expect(200);
    ...
    it("應該拒絕無效的範圍參數", async () => {
      const response = await request(app).get("/api/streamer/me/summary?range=invalid").expect(400);
    ...
    it("應該處理服務層錯誤", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/summary").expect(500);
  ...
  describe("GET /api/streamer/me/time-series", () => {
    ...
    it("應該成功取得時間序列資料（預設 day 粒度）", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/time-series").expect(200);

      expect(response.body).toEqual(mockTimeSeries);
      expect(streamerService.getStreamerTimeSeries).toHaveBeenCalledWith(
        "test-streamer-123",
        "30d",
        "day"
      );
    ...
    it("應該支援不同範圍參數", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/time-series?range=7d").expect(200);
    ...
    it("應該處理服務層錯誤", async () => {
      (streamerService.getStreamerTimeSeries as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );

      const response = await request(app).get("/api/streamer/me/time-series").expect(500);
  ...
  describe("GET /api/streamer/me/heatmap", () => {
    ...
    it("應該成功取得熱力圖資料", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/heatmap").expect(200);
    ...
    it("應該支援不同範圍參數", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/heatmap?range=7d").expect(200);
    ...
    it("應該支援 90 天範圍", async () => {
      ...
      const response = await request(app).get("/api/streamer/me/heatmap?range=90d").expect(200);
    ...
    it("應該拒絕無效的範圍參數", async () => {
      const response = await request(app).get("/api/streamer/me/heatmap?range=invalid").expect(400);
    ...
    it("應該處理服務層錯誤", async () => {
      (streamerService.getStreamerHeatmap as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );

      const response = await request(app).get("/api/streamer/me/heatmap").expect(500);
  ...
  describe("GET /api/streamer/me/subscription-trend", () => {
    ...
    it("應該成功取得訂閱趨勢資料", async () => {
      (subscriptionSyncService.getSubscriptionTrend as jest.Mock).mockResolvedValue(
        mockSubscriptionTrend
      );

      const response = await request(app).get("/api/streamer/me/subscription-trend").expect(200);

      expect(response.body).toEqual(mockSubscriptionTrend);
      expect(subscriptionSyncService.getSubscriptionTrend).toHaveBeenCalledWith(
        "test-streamer-123",
        "30d"
      );
    });
    ...
    it("應該處理服務層錯誤", async () => {
      (subscriptionSyncService.getSubscriptionTrend as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );

      const response = await request(app).get("/api/streamer/me/subscription-trend").expect(500);
  ...
  describe("POST /api/streamer/me/sync-subscriptions", () => {
    it("應該成功同步訂閱資料", async () => {
      ...
      const response = await request(app).post("/api/streamer/me/sync-subscriptions").expect(200);

      expect(response.body).toEqual({ message: "Subscription data synced successfully" });
      expect(subscriptionSyncService.syncSubscriptionSnapshot).toHaveBeenCalledWith(
        "test-streamer-123"
      );
    });
    ...
    it("應該處理找不到頻道的錯誤", async () => {
      ...
      const response = await request(app).post("/api/streamer/me/sync-subscriptions").expect(404);
    ...
    it("應該處理找不到 Token 的錯誤", async () => {
      ...
      const response = await request(app).post("/api/streamer/me/sync-subscriptions").expect(401);
    ...
    it("應該處理 Twitch API 未授權錯誤", async () => {
      ...
      const response = await request(app).post("/api/streamer/me/sync-subscriptions").expect(403);
    ...
    it("應該處理一般服務層錯誤", async () => {
      ...
      const response = await request(app).post("/api/streamer/me/sync-subscriptions").expect(500);
