...
export function ChartLoading({ message }: ChartLoadingProps) {
  const t = useTranslations("streamer.charts.states");
  return (
    <div className="flex items-center justify-center h-[300px]">
      <div className="flex flex-col items-center gap-3">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-purple-500"></div>
        <p className="text-sm text-gray-400">{message || t("loading")}</p>
      </div>
    </div>
  );
}
...
export function ChartError({ error, onRetry }: ChartErrorProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[300px] px-4">
      ...
      <p className="text-base sm:text-lg font-medium text-red-400 mb-2">
        {t("errorTitle")}
      </p>
    ...
    <div className="bg-yellow-900/20 border border-yellow-600/50 rounded-lg p-4 mb-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="font-semibold text-yellow-400">{t("comingSoon")}</h3>
          <p className="text-sm text-gray-300 mt-1">
            {t("limitedData", { current: currentDays, min: minDays })}
          </p>
          {onSync && (
            <button
              onClick={onSync}
