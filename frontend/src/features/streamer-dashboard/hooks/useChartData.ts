...
import { getStreamerTimeSeries, getStreamerHeatmap } from '@/lib/api/streamer';
import type { TimeSeriesDataPoint, HeatmapCell } from '@/lib/api/streamer';
...
export function useHeatmapData(range: ChartRange) {
  const { data, error, isLoading, mutate } = useSWR<HeatmapCell[]>(
