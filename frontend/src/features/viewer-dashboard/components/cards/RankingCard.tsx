...
>(({ watchPercentile, msgPercentile, ...props }, ref) => {
  ...
  return (
    <DashboardCardWrapper ref={ref} title={t("engagementRanking")} {...props}>
      <div className="flex flex-col gap-2 justify-start">
        <div>
          <div className="flex justify-between text-xs text-slate-400 mb-1">
            <span>{t("watchRank")}</span>
            <span className="text-white">
              {t("topPercent", { percent: (100 - watchPercentile).toFixed(1) })}
            </span>
          </div>
        ...
        <div>
          <div className="flex justify-between text-xs text-slate-400 mb-1">
            <span>{t("interactionRank")}</span>
            <span className="text-white">
              {t("topPercent", { percent: (100 - msgPercentile).toFixed(1) })}
            </span>
          </div>
          <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
            <div
