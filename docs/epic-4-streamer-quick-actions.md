...
- 使用 `@twurple/api` (ApiClient) 更新設定
- 方法：`apiClient.channels.updateChannelInfo()`
...
- 使用 `@twurple/api` (ApiClient) 獲取訂閱者列表
- 方法：`apiClient.subscriptions.getSubscriptionsPaginated()`
- 需要 `channel:read:subscriptions` OAuth 權限。
- 後端新增 `SubscriptionSnapshot` 資料表記錄每日快照。
- 收益預估公式：Tier 1 ($4.99 _ 50%)、Tier 2 ($9.99 _ 50%)、Tier 3 ($24.99 \* 50%)。
...
- 使用 `@twurple/eventsub` 監聽 Bits 事件 (EventSub)
- 事件：`channel.cheer`
