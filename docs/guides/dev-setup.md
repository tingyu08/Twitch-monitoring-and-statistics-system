...
本文件說明如何在本機啟動整個專案（前端 + 後端 + DB／排程骨架），以及最小可行的 CI / 測試流程，協助新成員與 Dev Agent 快速進入狀況。
...
> 若採單一 monorepo，可在根目錄使用 workspace（pnpm/yarn workspace 或 TurboRepo 等），此處先不強制規範，交由實際實作時決定。
...
> 以下為 **預期流程草稿**，實際指令需在 repo 建立後更新。目標是讓 Dev Agent 有一個清楚的「應該存在」的流程雛形。
...
> 首次啟動後端前，記得執行 DB schema／migrations，指令依 ORM 決定（例如 Prisma：`npx prisma migrate dev`）。
...
在真正上線前，建議至少建立以下 CI 步驟（以 GitHub Actions 為例，實際檔案可命名為 `.github/workflows/ci.yml`）：
