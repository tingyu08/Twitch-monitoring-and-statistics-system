...
**作為** 系統
**我想要** 有合理的資料表設計儲存開台與互動統計
**以便於** 能支援時間區間查詢與長期趨勢分析
...
**觀眾互動：** 7. `ViewerChannelDailyStat` - 觀眾每日觀看統計 8. `ViewerChannelMessage` - 觀眾留言記錄 9. `ViewerChannelMessageDailyAgg` - 留言每日彙總

**進階統計：** 10. `ViewerChannelLifetimeStats` - 觀眾全時段統計 11. `ViewerDashboardLayout` - 儀表板佈局設定

**分散式協調：** 12. `ListenerInstance` - 聊天監聽實例 13. `ChannelListenerLock` - 頻道監聽鎖

**隱私控制：** 14. `ViewerPrivacyConsent` - 隱私同意設定 15. `DeletionRequest` - 刪除請求 16. `ExportJob` - 資料匯出任務 17. `DataRetentionLog` - 資料保留日誌 18. `PrivacyAuditLog` - 隱私操作審計
