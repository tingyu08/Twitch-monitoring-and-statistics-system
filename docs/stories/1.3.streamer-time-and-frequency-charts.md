...
- [ ] Task 1（AC: 1）後端時間序列 API
  - [ ] 在後端實作 `GET /api/streamer/me/time-series?range=...&granularity=day|week` 端點
  - [ ] 依 `ChannelDailyStat` 等資料表計算每日/每週總開台時數與場數
  - [ ] 回傳結構需附上日期（或週起始日）、總時數、場數等欄位

- [ ] Task 2（AC: 2）後端 Heatmap / 時段分布資料
  - [ ] 設計並實作 `GET /api/streamer/me/heatmap?range=...` 等端點
  - [ ] 將一週 × 24 小時或區間的開台資訊彙整為矩陣資料結構
  - [ ] 確保輸出中包含星期、時段、值，以及必要的最大/最小參考值供前端畫色階

- [ ] Task 3（AC: 1, 2, 3）前端圖表元件實作
  - [ ] 在 `streamer-dashboard` feature 下建立 `TimeSeriesChart` 與 `HeatmapChart` 封裝
  - [ ] 使用選定的圖表庫（Recharts / ECharts）依 `front-end-architecture.md` 規劃實作
  - [ ] 實作 Tooltip、Legend、X/Y 軸標籤與單位

- [ ] Task 4（AC: 3, 4）互動與響應式
  - [ ] 依 `front-end-spec.md` 的 breakpoints 對圖表容器進行佈局調整
  - [ ] 確保 hover / focus 狀態符合 UX 文件的 motion 與可訪問性要求
  - [ ] 行動裝置上以垂直堆疊顯示，保留 Tooltip 與最關鍵數據

- [ ] Task 5（AC: 5）邊界情境與錯誤顯示
  - [ ] 後端在資料不足時回傳明確狀態（空集合、旗標），避免 500 錯誤
  - [ ] 前端偵測空資料時顯示 EmptyState 與簡短文字說明
  - [ ] 若為 API 限制 / 資料延遲等 case，顯示 Info Banner（沿用共用 `Banner` 元件）
