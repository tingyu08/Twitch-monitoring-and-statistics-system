...
- [ ] Task 1.3: 分級保留 Cron Job
  - [x] 新增 `src/jobs/data-retention.job.ts`
  - [x] 每日凌晨 3 點執行
  - [x] 邏輯：
    - 查詢 `lastActiveAt < now() - 365 days` 的觀眾
    - 發送 Email 提醒（330 天、355 天）
    - 自動執行軟刪除（365 天）
    - 執行冷靜期結束的最終匿名化（7 天後）
...
- [ ] Task 3.4: 匯出 API 端點
  ...
  - [x] 新增 Email 通知（當匯出完成時）

### Task 4（AC 1, 6）前端：隱私設定 UI

- [ ] Task 4.1: 建立隱私設定頁面
  - [x] 新增 `src/app/settings/privacy/page.tsx`
  - [x] 整合在現有 Settings 頁面的左側選單
...
| 庫 | 用途 | 版本 | 備註 |
|----|------|------|------|
| `bull` | 非同步任務佇列（資料匯出） | ^4.12 | 需要 Redis |
...
- ✅ Story 2.3 完成（留言統計）
- ✅ Story 2.4 完成（足跡總覽）
- ⏳ Redis 伺服器（Bull Queue 依賴）
- ⏳ Email 服務（SendGrid 或 SMTP）
- ⏳ 檔案儲存（本地或 S3）
...
- [ ] 細粒度開關正常運作，即時生效
- [ ] 同意橫幅在首次登入時正確顯示
- [ ] 資料匯出生成 ZIP 包含 JSON + CSV + HTML
- [ ] 軟刪除流程正確執行（7 天冷靜期 + 匿名化）
- [ ] 分級保留 Cron 正確識別並通知非活躍用戶
- [ ] 隱私設定 API 回應時間 < 200ms（p95）
- [ ] 匯出任務非同步處理不阻塞主執行緒
- [ ] 刪除後無法查詢個人識別資訊
- [ ] 實況主端聚合統計不受刪除影響
- [ ] 後端單元測試通過率 100%
